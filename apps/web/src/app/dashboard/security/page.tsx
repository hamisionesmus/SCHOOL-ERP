...
interface AlertsPage {
  data: SecurityAlert[];
  meta: { page: number; pageSize: number; total: number; unacknowledgedCount: number };
}
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle className="flex items-center gap-2">
          <ShieldAlert size={18} />
          Security Alerts
          {data && data.meta.unacknowledgedCount > 0 && (
            <span className="rounded-full bg-rose-100 px-2 py-0.5 text-xs font-semibold text-rose-700">
              {data.meta.unacknowledgedCount} unacknowledged
            </span>
          )}
        </CardTitle>
      </CardHeader>
